...
class AproveIKScreen extends StatefulWidget {
  @override
  _AproveIKScreenState createState() => _AproveIKScreenState();
}

class _AproveIKScreenState extends State<AproveIKScreen> {
  ...
  Future<void> retrievePosts() async {
    try {
      ...
      ApiResponse response = await getAdminIzinKeluar();
  ...
  void onViewIzinKeluar(RequestIzinKeluar izinKeluar) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text("Izin Keluar Details"),
          content: Column(
  ...
  void _approveIzinKeluar(int id) async {
    try {
      ...
      ApiResponse response = await approveLeaveRequest(id);
      print("Response message: ${response.data}");

      if (response.error == null) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Reqyest Izin Keluar sudah Di Approve'),
        ));
  ...
  void _rejectedIzinKeluar(int id) async {
    try {
      ...
      ApiResponse response = await rejectedLeaveRequest(id);
      print("Response message: ${response.data}");

      if (response.error == null) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Reqyest Izin Keluar sudah Di Reject'),
        ));
